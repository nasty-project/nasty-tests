...
if [[ "$HAS_HOST" != true ]]; then
  ...
  echo "  --create-pool      Auto-create pool on available devices if it doesn't exist"
    ...
    echo "  --skip-rbac        Skip management-role and scoped-token tests"
    echo "  --skip-protocol-auth  Skip authenticated/denied protocol probes"
    echo "  --only-security    Run only RBAC and protocol authorization tests"
    echo "  --block-persistence  Restart engine and verify block exports"
    echo "  --only-block-persistence  Run only block-export restart tests"
    echo "  --restart-timeout SEC  Restart/API timeout (default 180)"
